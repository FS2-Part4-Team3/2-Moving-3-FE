...
export function ModeToggle() {
  ...
  return (
    <button
      ...
      className={`lg:w-[7.5rem] lg:h-[4rem] sm:h-[2rem] sm:w-[4rem] rounded-[3rem] gap-[0.5rem] flex items-center border border-dark-t lg:pl-[0.3rem] lg:dark:pr-[0.3rem] lg:dark:pl-0 sm:pl-[0.2rem] sm:dark:pr-[0.2rem] sm:dark:pl-0 dark:justify-end
        ${isDark ? 'bg-dark-bg text-dark-t' : 'bg-gray-100 text-black-400'}
      `}
    >
      <span className={`text-[1rem] font-semibold ${isDark ? 'lg:block sm:hidden opacity-100' : 'opacity-0 hidden'}`}>NIGHT</span>
      <div
      ...
      <span className={`text-[1rem] font-bold ${isDark ? 'opacity-0 hidden' : 'opacity-100 lg:block sm:hidden'}`}>DAY</span>
    </button>
  );
}
