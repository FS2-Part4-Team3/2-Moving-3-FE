    ...
    <div className="flex lg:gap-0 md:gap-[5.6rem] md:flex-row justify-center items-center sm:flex-col sm:gap-[4rem]">
      <div className="flex flex-col justify-center gap-[1.5rem] lg:hidden sm:block">
        <div className="flex flex-row gap-[0.8rem] justify-center items-center">
          <p className="text-black-400 dark:text-dark-t text-[4rem] leading-[4.773rem] font-bold">{score.toFixed(1)}</p>
      ...
      <div
        className="flex flex-row bg-background-200 dark:bg-dark-p lg:rounded-[3.2rem] sm:rounded-[2.4rem] lg:px-[2.5rem] lg:py-[4rem] lg:gap-[6rem] sm:px-[1.8rem] sm:py-[1.6rem] sm:gap-[0.6rem] lg:w-full lg:h-[29.6rem] sm:w-[32.7rem] sm:h-[17.6rem] items-center justify-center"
        data-testid="main-chart"
      >
        <div className="flex flex-col justify-center gap-[1.5rem] lg:block sm:hidden">
          <div className="flex flex-row gap-[0.8rem] justify-center items-center">
            <p className="text-black-400 dark:text-dark-t text-[6.4rem] leading-[7.638rem] font-bold">{score.toFixed(1)}</p>
        ...
        <div className="flex lg:gap-[1.4rem] sm:gap-[0.6rem] flex-col justify-between">
          {chartData.map(item => (
            <div key={item.score} className="flex flex-row lg:gap-[3rem] sm:gap-[1.6rem] items-center justify-start">
              <p
                className={`text-black-300 dark:text-dark-t lg:text-[2rem] lg:leading-[3.2rem] sm:text-[1.4rem] leading-[2.4rem] w-[3.6rem] ${
              ...
              <p
                className={`text-gray-300 dark:text-dark-t lg:text-[2rem] lg:leading-[3.2rem] sm:text-[1.4rem] sm:leading-[2.4rem] lg:w-[4.2rem] sm:w-[3.6rem] ${
                  item.score === 5 ? 'font-bold' : 'font-medium'
                }`}
              >
                {item.count}
