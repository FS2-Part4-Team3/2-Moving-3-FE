...
import arrow_down_gray from '@/../public/assets/common/dropdown/chevron-down_gray.svg';
      ...
      <div className="flex flex-col gap-[1.6rem] relative">
        ...
        <div
          className={`lg:w-full lg:h-fit sm:w-fit sm:h-[3.6rem] gap-[0.6rem] lg:rounded-[1.6rem] sm:rounded-[0.8rem] border lg:py-[1.6rem] lg:px-[2.4rem] sm:py-[0.6rem] sm:pr-[1rem] sm:pl-[1.4rem] flex items-center justify-between cursor-pointer mb-[0.8rem] ${
            isDropdownOpen ? 'border-blue-300 bg-blue-50 dark:bg-dark-p' : 'border-gray-100 dark:bg-dark-p'
          }`}
          onClick={() => setIsDropdownOpen(!isDropdownOpen)}
        >
          <p
            className={`font-medium lg:text-[1.8rem] sm:text-[1.4rem] lg:leading-[2.6rem] sm:leading-[2.4rem] ${
              isDropdownOpen ? 'text-blue-300' : 'text-black-400 dark:text-dark-t'
            }`}
          >
            {selectedMenu || '전체'}
          </p>
          <Image
            src={
              theme === 'dark' ? (isDropdownOpen ? clicked_arrow : arrow_down_gray) : isDropdownOpen ? clicked_arrow : arrow_down
            }
            alt="arrow"
            width={36}
            height={36}
          ...
          >
            <div className="h-fit lg:w-[32.8rem] sm:w-[15rem]">
              {reviewAnalysisTypes.map((region, index) => (
                <div
                  key={index}
                  className="lg:w-[16.4rem] lg:h-[6.4rem] lg:py-[1.6rem] lg:px-[2.4rem] sm:w-fit sm:h-[3.6rem] sm:py-[0.6rem] sm:px-[1.4rem] font-medium lg:text-[1.8rem] sm:text-[1.4rem] lg:leading-[2.6rem] sm:leading-[2.4rem] text-black-400 dark:text-dark-t cursor-pointer"
